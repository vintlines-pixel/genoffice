...
export const strings = {
  zh: {
    ...
    setAiTestOk: '连接成功',
    setAiTestFail: '连接失败',
    setAiImageGen: '图像生成 API（可选）',
    setAiImageGenHint: 'OpenAI 兼容图像端点。配置后 AI 生图将使用你自己的服务，无需登录 Genspark',
    setAiSerperKey: '搜索 API Key（Serper）',
    setAiSerperKeyHint: '可选。Genspark 云工具关闭时，网页/图片搜索将通过 Serper（google.serper.dev）进行',
  ...
  en: {
    ...
    setAiImageGen: 'Image generation API (optional)',
    setAiImageGenHint: 'OpenAI-compatible images endpoint. When configured, AI image generation uses your own service instead of Genspark — no sign-in required',
    setAiSerperKey: 'Search API key (Serper)',
    setAiSerperKeyHint: 'Optional. When Genspark cloud tools are off, web/image search runs through Serper (google.serper.dev)',
  ...
  ja: {
    ...
    setAiImageGen: '画像生成 API（任意）',
    setAiImageGenHint: 'OpenAI 互換の画像エンドポイント。設定すると、AI 画像生成は Genspark にログインせず自分のサービスを使用します',
    setAiSerperKey: '検索 API キー（Serper）',
    setAiSerperKeyHint: '任意。Genspark クラウドツールがオフのとき、ウェブ/画像検索は Serper（google.serper.dev）経由で実行されます',
  ...
  ko: {
    ...
    setAiTestOk: '연결 성공',
    setAiTestFail: '연결 실패',
    setAiImageGen: '이미지 생성 API(선택)',
    setAiImageGenHint: 'OpenAI 호환 이미지 엔드포인트. 설정하면 AI 이미지 생성이 Genspark 로그인 없이 자체 서비스를 사용합니다',
    setAiSerperKey: '검색 API 키(Serper)',
    setAiSerperKeyHint: '선택 사항. Genspark 클라우드 도구가 꺼져 있으면 웹/이미지 검색이 Serper(google.serper.dev)를 통해 실행됩니다',
  ...
  fr: {
    ...
    setAiImageGen: 'API de génération visuelle (optionnel)',
    setAiImageGenHint: 'Endpoint images compatible OpenAI. Une fois configuré, la génération utilise votre propre service, sans connexion à Genspark',
    setAiSerperKey: 'Clé API de recherche (Serper)',
    setAiSerperKeyHint: 'Optionnel. Lorsque les outils cloud Genspark sont désactivés, la recherche web et images passe par Serper (google.serper.dev)',
  ...
  de: {
    ...
    setAiImageGen: 'Bildgenerierungs-API (optional)',
    setAiImageGenHint: 'OpenAI-kompatibler Bilder-Endpunkt. Bei Konfiguration nutzt die Bildgenerierung deinen eigenen Dienst statt Genspark — keine Anmeldung nötig',
    setAiSerperKey: 'Such-API-Schlüssel (Serper)',
    setAiSerperKeyHint: 'Optional. Bei ausgeschalteten Genspark-Cloud-Tools läuft die Web- und Bildersuche über Serper (google.serper.dev)',
    setAiGskTools: 'Genspark-Cloud-Tools',
    setAiGskToolsDesc:
  ...
  es: {
    ...
    setAiImageGen: 'API de generación de imágenes (opcional)',
    setAiImageGenHint: 'Endpoint de imágenes compatible con OpenAI. Si se configura, la generación de imágenes usa tu propio servicio en lugar de Genspark, sin inicio de sesión',
    setAiSerperKey: 'Clave API de búsqueda (Serper)',
    setAiSerperKeyHint: 'Opcional. Cuando las herramientas cloud de Genspark están desactivadas, la búsqueda web y de imágenes usa Serper (google.serper.dev)',
  ...
  th: {
    ...
    setAiImageGen: 'API สร้างภาพ (ไม่บังคับ)',
    setAiImageGenHint: 'ปลายทางรูปภาพที่รองรับ OpenAI เมื่อตั้งค่าแล้ว การสร้างภาพ AI จะใช้บริการของคุณแทน Genspark โดยไม่ต้องเข้าสู่ระบบ',
    setAiSerperKey: 'คีย์ API สำหรับค้นเว็บ (Serper)',
    setAiSerperKeyHint: 'ไม่บังคับ เมื่อเครื่องมือคลาวด์ Genspark ปิดอยู่ การค้นเว็บ/รูปภาพจะใช้ Serper (google.serper.dev)',
  ...
  id: {
    ...
    setAiImageGen: 'API pembuatan gambar (opsional)',
    setAiImageGenHint: 'Endpoint gambar kompatibel OpenAI. Jika dikonfigurasi, pembuatan gambar AI memakai layanan Anda sendiri, tanpa login Genspark',
    setAiSerperKey: 'Kunci API pencarian (Serper)',
    setAiSerperKeyHint: 'Opsional. Saat alat cloud Genspark mati, pencarian web/gambar berjalan melalui Serper (google.serper.dev)',
  ...
  ru: {
    ...
    setAiImageGen: 'API генерации изображений (необязательно)',
    setAiImageGenHint: 'Совместимый с OpenAI эндпоинт изображений. После настройки генерация изображений использует ваш сервис, без входа в Genspark',
    setAiSerperKey: 'Ключ API поиска (Serper)',
    setAiSerperKeyHint: 'Необязательно. Когда облачные инструменты Genspark отключены, веб-поиск и поиск изображений выполняются через Serper (google.serper.dev)',
  ...
  ar: {
    ...
    setAiImageGen: 'واجهة توليد الصور (اختياري)',
    setAiImageGenHint: 'نقطة نهاية صور متوافقة مع OpenAI. عند الإعداد، يستخدم توليد الصور خدمتك بدلاً من Genspark دون تسجيل دخول',
    setAiSerperKey: 'مفتاح API للبحث (Serper)',
    setAiSerperKeyHint: 'اختياري. عند إيقاف أدوات Genspark السحابية، يتم البحث في الويب والصور عبر Serper (google.serper.dev)',
  ...
  pt: {
    ...
    setAiImageGen: 'API de geração de imagens (opcional)',
    setAiImageGenHint: 'Endpoint de imagens compatível com OpenAI. Quando configurado, a geração de imagens usa o seu serviço, sem login no Genspark',
    setAiSerperKey: 'Chave de API de pesquisa (Serper)',
    setAiSerperKeyHint: 'Opcional. Quando as ferramentas cloud do Genspark estão desativadas, a pesquisa de web e imagens usa a Serper (google.serper.dev)',
  ...
  it: {
    ...
    setAiImageGen: 'API di generazione immagini (facoltativa)',
    setAiImageGenHint: 'Endpoint immagini compatibile OpenAI. Una volta configurato, la generazione di immagini usa il tuo servizio invece di Genspark, senza accesso',
    setAiSerperKey: 'Chiave API di ricerca (Serper)',
    setAiSerperKeyHint: 'Facoltativa. Quando gli strumenti cloud Genspark sono disattivati, la ricerca web e immagini avviene tramite Serper (google.serper.dev)',
  ...
  pl: {
    ...
    setAiImageGen: 'API generowania obrazów (opcjonalne)',
    setAiImageGenHint: 'Punkt końcowy obrazów zgodny z OpenAI. Po skonfigurowaniu generowanie obrazów korzysta z własnej usługi, bez logowania do Genspark',
    setAiSerperKey: 'Klucz API wyszukiwania (Serper)',
    setAiSerperKeyHint: 'Opcjonalne. Gdy narzędzia chmurowe Genspark są wyłączone, wyszukiwanie stron i obrazów odbywa się przez Serper (google.serper.dev)',
  ...
  nl: {
    ...
    setAiImageGen: 'API voor beeldgeneratie (optioneel)',
    setAiImageGenHint: 'OpenAI-compatibel afbeeldingseindpunt. Indien geconfigureerd gebruikt beeldgeneratie je eigen service, zonder inloggen bij Genspark',
    setAiSerperKey: 'Zoek-API-sleutel (Serper)',
    setAiSerperKeyHint: 'Optioneel. Als Genspark-cloudtools uit staan, verloopt het zoeken naar web en afbeeldingen via Serper (google.serper.dev)',
  ...
  ms: {
    ...
    setAiImageGen: 'API penjanaan imej (pilihan)',
    setAiImageGenHint: 'Endpoint imej serasi OpenAI. Apabila dikonfigurasi, penjanaan imej menggunakan perkhidmatan anda, tanpa log masuk Genspark',
    setAiSerperKey: 'Kunci API carian (Serper)',
    setAiSerperKeyHint: 'Pilihan. Apabila alat cloud Genspark dimatikan, carian web/imej dijalankan melalui Serper (google.serper.dev)',
  ...
  he: {
    ...
    setAiImageGen: 'API ליצירת תמונות (אופציונלי)',
    setAiImageGenHint: 'נקודת קצה לתמונות תואמת OpenAI. לאחר הגדרה, יצירת תמונות AI משתמשת בשירות שלך במקום Genspark, ללא צורך בהתחברות',
    setAiSerperKey: 'מפתח API לחיפוש (Serper)',
    setAiSerperKeyHint: 'אופציונלי. כשכלי הענן של Genspark כבויים, החיפוש באינטרנט ובתמונות עובר דרך Serper (google.serper.dev)',
  ...
  hi: {
    ...
    setAiImageGen: 'इमेज जनरेशन API (वैकल्पिक)',
    setAiImageGenHint: 'OpenAI-संगत इमेज एंडपॉइंट। कॉन्फ़िगर करने पर AI इमेज जनरेशन आपकी सेवा उपयोग करता है, Genspark लॉगिन ज़रूरी नहीं',
    setAiSerperKey: 'सर्च API कुंजी (Serper)',
    setAiSerperKeyHint: 'वैकल्पिक। जब Genspark क्लाउड टूल बंद हों, तो वेब/इमेज सर्च Serper (google.serper.dev) से होता है',
  ...
  'zh-TW': {
    ...
    setAiTestOk: '連線成功',
    setAiTestFail: '連線失敗',
    setAiImageGen: '圖像生成 API（選填）',
    setAiImageGenHint: 'OpenAI 相容圖像端點。設定後 AI 生圖將使用你自己的服務，無需登入 Genspark',
    setAiSerperKey: '搜尋 API Key（Serper）',
    setAiSerperKeyHint: '選填。Genspark 雲端工具關閉時，網頁/圖片搜尋將透過 Serper（google.serper.dev）進行',
